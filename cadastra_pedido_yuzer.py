...
def monta_det(planilha,sheet_name):
    ...
    for index, p in df.iterrows():
        ...
        imposto =  {'cofins_padrao': {'cod_sit_trib_cofins': tributacao['pis_cofins_cst']},
              'icms': {'cod_sit_trib_icms': tributacao['icms_cst']},
              'pis_padrao': {'cod_sit_trib_pis': tributacao['pis_cofins_cst']}
              }
        
        produto = {'cfop': tributacao['cfop'],
                'codigo_produto': p['CODIGO OMIE'], #Consultar na propria lista do OMIE
                'codigo_produto_integracao': p['CODIGO OMIE'],
                'descricao': f'{p['Produto'][:115]}__',
                'ncm': tributacao['ncm'],
...
tipo_de_venda = 'BEBIDAS' #Informação vem da planilha
conta_corrente_micare =11831850443
filepath = r'D:\Projetos\2J\produtos_yuzer_v3.xlsx'
...
det = monta_det(filepath,'Bebidas')
